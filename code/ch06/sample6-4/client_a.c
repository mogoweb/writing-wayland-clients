...
static void draw_frame(struct state *state) {
    ...
    cairo_text_extents(cr, "Hello Client A!", &extents);
    cairo_move_to(cr, state->width/2.0 - extents.width/2.0, state->height/2.0);
    cairo_show_text(cr, "Hello Client A!");
...
static void zone_handle(void *data, struct xx_zone_v1 *xx_zone_v1, const char *handle)
{
    ...
    if (fwrite(handle, 1, len, fp) != len) {
        fprintf(stderr, "Failed to write handle.id: %s\n", strerror(errno));
        fclose(fp);
        return;
    }
...
int main()
{
    ...
    // 设置窗口标题
    xdg_toplevel_set_title(state.xdg_toplevel, "Wayland Client A");
